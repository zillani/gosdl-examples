...
func (self *Ball) Rect() *sdl.Rect {
	size := int32(self.radius * 2)
	x := self.vector.X - self.radius
	y := self.vector.Y - self.radius
	return &sdl.Rect{X: int32(x), Y: int32(y), W: size, H: size}
}
...
func (self *World) HandleEvents() {
	for self.running {
		...
		case *sdl.KeyboardEvent:
			...
			case sdl.KMOD_LCTRL:
				keys += "Left Control"
			case sdl.Keycode(sdl.K_p):
				self.pause = !self.pause
			case sdl.Keycode(sdl.K_j):
				self.Paddle.Go(0, self.Paddle.vector.Y+self.Paddle.speed)
			case sdl.Keycode(sdl.K_k):
				self.Paddle.Go(0, self.Paddle.vector.Y-self.Paddle.speed)
			case sdl.Keycode(sdl.K_q):
				self.running = false
/*			case sdl.MOUSEMOTION:
				motion := sdl.MOUSEMOTION()
...
func (self *World) Draw() {
	...
	center := &sdl.Rect{X: int32(self.Width/2) - 1, Y: 0, H: 200, W: 2}
...
func (self *Score) Draw(world *World) {
	pRect := &sdl.Rect{
		X: int32(world.Paddle.width + world.Paddle.vector.X),
		Y: 3, W: 3, H: 3,
	}
	...
	eRect := &sdl.Rect{
		X: int32(world.Enemy.vector.X - world.Enemy.width),
		Y: int32(world.Height - 6), W: 3, H: 3,
	}
